<template>
    <div>
        ...
        <div v-if="oppurtunities">
            <DataTable :value="oppurtunities" :resizableColumns="true" class="p-datatable-md" columnResizeMode="fit"
                showGridlines responsiveLayout>
                ...
                <DataColumn field="action" header="Action">
                    <template #body="slotProps">
                        <InputButton icon="pi pi-pencil" class="p-button-rounded p-button-success"
                            @click="showEditOppurtunity(slotProps.data)" />
                        <br>
                        <InputButton icon="pi pi-trash" class="p-button-rounded p-button-danger"
                            @click="showNotSupported()" />
                    </template>
                </DataColumn>
            </DataTable>
        </div>
    ...
    <ViewDialog v-model:visible="newOppurtunityDialog" header="New Oppurtunity" :modal="true" class="p-fluid">
        ...
        <div class="field">
            ...
            <InputText id="name" v-model.trim="BrandnewOppurtunity.name" required="true" autofocus
                :class="{ 'p-invalid': submitted && !BrandnewOppurtunity.name }" />
            <small class="p-error" v-if="submitted && !oppurtunity.name">Name is required.</small>
        </div>

        <div class="field">
            <label for="status">Status</label>
            <ListBox id="status" v-model="BrandnewOppurtunity.status" :options=oppurtunityStatusList required="true"
                autofocus :class="{ 'p-invalid': submitted && !BrandnewOppurtunity.name }" />
    ...
    <ViewDialog v-model:visible="editOppurtunityDialog" header="Edit Oppurtunity" :modal="true" class="p-fluid">
        ...
        <div class="field">
            <label for="name">Name</label>
            <InputText id="name" v-model.trim="oppurtunity.name" required="true" autofocus
                :class="{ 'p-invalid': submitted && !oppurtunity.name }" />
            <small class="p-error" v-if="submitted && !oppurtunity.name">Name is required.</small>
        </div>

        <div class="field">
            <label for="">Status</label>
            <ListBox id="status" v-model="oppurtunity.status" :options=oppurtunityStatusList required="true" autofocus
                :class="{ 'p-invalid': submitted && !oppurtunity.name }" />
